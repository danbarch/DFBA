...
The `a0` and `b0` arguments represent the $\alpha_0$ and $\beta_0$ shape parameters for the prior beta distribution for the Bayesian distribution-free analyses; the default values `a0 = 1` and `b0 = 1` correspond to a non-informative prior distribution. Other values of `a0` and `bo` may be specified to use a different prior distribution, for example, to use an informative prior.
